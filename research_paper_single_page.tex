...
\usepackage{booktabs}
\usepackage{hyperref}
\usepackage[margin=0.48in]{geometry} % Tight margins to keep it on one page in single-column
\usepackage{tikz}
\usetikzlibrary{shapes.geometric, arrows, positioning}

\begin{document}

\title{\large \textbf{Multi-Fold Multimodal Fusion for Enhanced Cardiovascular Risk Stratification}}
...
\begin{tikzpicture}[
    ...
    box/.style={draw, rectangle, rounded corners=3pt, fill=blue!5, text width=3.4cm, align=center, minimum height=0.6cm, font=\scriptsize},
    decision/.style={draw, diamond, aspect=2.2, fill=yellow!10, text width=1.5cm, align=center, font=\scriptsize},
    io/.style={draw, trapezium, trapezium left angle=75, trapezium right angle=105, fill=green!10, text width=2.4cm, align=center, font=\scriptsize},
    ...
    \node (ecg) [box, left=of cxr, xshift=-0.2cm] {\textbf{12-Lead ECG}\\1D ResNet Classifier};
    \node (echo) [box, right=of cxr, xshift=0.2cm] {\textbf{Echo-Analysis}\\CNN-LSTM Video Classifier};
    ...
    % Branching to Fold 2
    \draw [arrow] (engage.south) -- ++(0,-0.28) -| (ecg.north);
    \draw [arrow] (engage.south) -- (cxr.north);
    \draw [arrow] (engage.south) -- ++(0,-0.28) -| (echo.north);
    
    % Merging to Fusion
    \draw [arrow] (ecg.south) -- ++(0,-0.28) -| (fusion.north);
    \draw [arrow] (cxr.south) -- (fusion.north);
    \draw [arrow] (echo.south) -- ++(0,-0.28) -| (fusion.north);
    
    \draw [arrow] (fusion) -- (archive);
\end{tikzpicture}
\caption{Flowchart of the sequential triage gateway with horizontal modality branching.}
\label{fig:flowchart}
\end{figure}
