...
\tikzstyle{box} = [draw, rectangle, rounded corners=3pt, fill=blue!5, text width=2.4cm, align=center, minimum height=0.6cm, font=\scriptsize]
\tikzstyle{decision} = [draw, diamond, aspect=2, fill=yellow!10, text width=1.3cm, align=center, font=\scriptsize]
\tikzstyle{io} = [draw, trapezium, trapezium left angle=75, trapezium right angle=105, fill=green!10, text width=1.8cm, align=center, font=\scriptsize]
\tikzstyle{arrow} = [thick,->,>=stealth]

\begin{document}

\title{Multi-Fold Multimodal Fusion for Enhanced Cardiovascular Risk Stratification: A Secure Clinical Workstation Approach}

\author{\IEEEauthorblockN{Riya Moorjani}
\IEEEauthorblockA{\textit{Department of Clinical AI} \\
\textit{Precision Clinical Technologies}\\
Email: riyamoorjani2504@gmail.com}
}
...
\begin{tikzpicture}[node distance=1.1cm, auto]
    ...
    \node (discharge) [io, right of=gate, xshift=1.8cm] {Standard Discharge / Observation};
    \node (engage) [box, below of=gate, yshift=-0.4cm] {Engage Fold 2 Gate};
    ...
    \node (output) [box, below of=fusion] {Encrypted SQLite Audit Archive (AES-256)};
...
\begin{tikzpicture}[node distance=0.8cm, auto]
    % Modality Vectors
    \node (tab) [draw, rectangle, fill=red!10, minimum width=1.1cm] {\scriptsize $T$};
    \node (ecg) [draw, rectangle, fill=orange!10, right of=tab, xshift=0.5cm, minimum width=1.1cm] {\scriptsize $E$};
    \node (cxr) [draw, rectangle, fill=yellow!10, right of=ecg, xshift=0.5cm, minimum width=1.1cm] {\scriptsize $X$};
    \node (echo) [draw, rectangle, fill=purple!10, right of=cxr, xshift=0.5cm, minimum width=1.1cm] {\scriptsize $V_{\text{echo}}$};
    \node (nlp) [draw, rectangle, fill=green!10, right of=echo, xshift=0.5cm, minimum width=1.1cm] {\scriptsize $N$};

    % Concatenated Vector
    \node (concat) [draw, rectangle, fill=blue!10, below of=cxr, yshift=-0.5cm, minimum width=6.5cm, minimum height=0.4cm] {\scriptsize Fused Vector $F \in \mathbb{R}^{110}$};

    % Adapter Head
    \node (fc1) [draw, rectangle, fill=blue!20, below of=concat, yshift=-0.2cm, minimum width=4.5cm] {\scriptsize Fully Connected Layer (ReLU)};
    \node (output) [draw, rectangle, fill=red!20, below of=fc1, yshift=-0.2cm] {\scriptsize Unified Risk Index $\in [0,1]$};

    % Connections
    \draw [arrow] (tab.south) -- ++(0,-0.3) -| (concat.north);
    \draw [arrow] (ecg.south) -- ++(0,-0.3) -| (concat.north);
    \draw [arrow] (cxr.south) -- (concat.north);
    \draw [arrow] (echo.south) -- ++(0,-0.3) -| (concat.north);
    \draw [arrow] (nlp.south) -- ++(0,-0.3) -| (concat.north);
...
\bibitem{echo1} J. Ostrogna et al., ``Spatiotemporal convolutional neural networks for automated echocardiography segmentation and ejection fraction estimation,'' \textit{Medical Image Analysis}, vol. 68, p. 101890, 2021.
